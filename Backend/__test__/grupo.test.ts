...
describe("GET /api/cursos/:idCurso/grupos", () => {
    ...
    test("1. Obtener todos los grupos de un curso", async () => {
        ...
        expect(res.body.result[0].cursoId).toBe(curso.id);
        expect(res.body.result[1].cursoId).toBe(curso.id);
    }, 15000);
    ...
    test("3. Obtener todos los grupos de un curso con un integrante: ?integrante=", async () => {
        ...
        expect(res.body.result[0].integrantesModel).toEqual(expect.arrayContaining([expect.objectContaining({ nombre: integrante.nombre })]));
        expect(res.body.result[1].integrantesModel).toEqual(expect.arrayContaining([expect.objectContaining({ nombre: integrante.nombre })]));
    ...
    test("4. Obtener todos los grupos de un curso con un integrante: ?integrante= y limit correcto", async () => {
        ...
        expect(res.body.result.length).toBeLessThanOrEqual(limit);
        expect(res.body.result[0].cursoId).toBe(curso.id);
        expect(res.body.result[1].cursoId).toBe(curso.id);
        // del array integrantesModel, espero encontrar un integrante que se llame integrante.nombre
        expect(res.body.result[0].integrantesModel).toEqual(expect.arrayContaining([expect.objectContaining({ nombre: integrante.nombre })]));
